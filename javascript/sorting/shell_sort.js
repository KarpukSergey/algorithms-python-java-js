...
function shell_sort(list){
    let distance = Math.floor(list.length / 2);
    while(distance > 0){
        for(let i = distance; i < list.length; i++){
            let temp = list[i];
            let j = i
            // Сортировка подсписка для текущего значения дистанции
            // Sort the sublist for the current distance value
            while(j >= distance && list[j - distance] > temp){
...
let list = [29, 1, 15, 4, 89, 12, 45, 3, 9]

console.log(shell_sort(list))