...
function IntPolsearch(list, x){
    let idx0 = 0;
    let idxn = (list.length - 1);
    let found = false;
    while(idx0 < idxn && x >= list[idx0] && x <= list[idxn]){
        // Looking for the middle point
        // Ищем срединную точку
        let mid = idx0 + Math.floor((Math.floor(idxn - idx0) / (list[idxn] - list[idx0])) * (x - list[idx0]))
...
let list = [12, 33, 55, 99, 156, 987];
console.log(IntPolsearch(list, 99))