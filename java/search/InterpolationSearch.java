...
public class InterpolationSearch {
     ...
     public static boolean IntPolsearch(int[] list, int x){
        int idx0 = 0;
        int idxn = list.length - 1;

        while (idx0 <= idxn && x >= list[idx0] && x <= list[idxn]) {
            if (idx0 == idxn) { // один элемент остался
                return list[idx0] == x;
            }

            // Looking for the middle point
            // Ищем срединную точку
            int mid = idx0 + (int)(((long)(idxn - idx0) * (x - list[idx0])) 
                                  / (list[idxn] - list[idx0]));
    ...
    public static void main(String[] args){
        int[] list = {12, 33, 55, 99, 156, 987};
        System.out.println(IntPolsearch(list, 99));
    }
}
